...
| `verify` | Re-checks one finding against current HEAD, records an evidenced attack tree, runs three isolated attempts, scores five fixed evidence criteria, gates the verdict on host-matched design controls, and applies deterministic Medium severity caps for held authorization or sandbox controls |
