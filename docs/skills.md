...
| `verify` | Re-checks one finding against current HEAD, records an evidenced attack tree, runs three isolated attempts, scores five fixed evidence criteria, and records a non-scored bypass assessment for every host-matched design control. A held authorization or sandbox control deterministically caps severity at Medium; unresolved or unavailable control evidence marks calibration incomplete without lowering severity. |
...
| `verify` | Verification result and miss-count update on one Finding. Reconciled held authorization and sandbox controls also apply deterministic Medium severity caps; unknown control inputs only mark calibration incomplete. |
