...
func TestFindingShow_rendersLatestVerificationScore(t *testing.T) {
	...
	body := w.Body.String()
	for _, want := range []string{"Verification history", "confirmed", "100%", "Attack path", "reachable", "Trigger parser panic", "AT2", "Reach parser sink", "PoC well formed", "#1", "same panic", fmt.Sprintf("scan #%d", verifyScan.ID)} {
		if !strings.Contains(body, want) {
