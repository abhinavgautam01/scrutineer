...
func TestPackageManagerModeReports(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"missing threat model", audit, `{"review_status":"missing-threat-model","findings":[],"notes":"install reaches acquire; domain threat model is missing."}`, true},
		...
		{"finding without model", audit, `{"review_status":"missing-threat-model","findings":[` + finding + `],"notes":"Model absent.",` + evidence + `}`, false},
