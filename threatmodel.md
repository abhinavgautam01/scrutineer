...
The agent CLIs are release-and-SHA-pinned per architecture: `claude-code@2.1.259`, Codex, OpenCode, and Copilot. Other tool versions are pinned: `semgrep==1.167.0`, `bandit==1.9.4`, `git-pkgs@v0.19.0`, `zizmor@1.30.0`. The final stage is `debian:trixie-slim`; the `golang:1.27.1-trixie` and `rust:1.98-trixie` builder stages are pinned by sha256 digest. The container runs as non-root user `runner`. The runner image is built in CI, smoke-tested, and published to GHCR; users pull a known-good artifact rather than rebuilding against live registries.
